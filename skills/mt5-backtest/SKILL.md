...
test -f "$MT5_BASE/config/accounts.dat" || echo "WARNING: No cached broker credentials. Run ./scripts/login.sh"
```
...
6. Saves password to `config/credentials.env` (chmod 600, gitignored)
7. Auto-backs up credentials on success

### Login Management Commands

```bash
./scripts/login.sh                          # Interactive login (prompts step by step)
./scripts/login.sh <LOGIN> <PASS> <SERVER>  # Direct login with all args
./scripts/login.sh --status                 # Check credential status
./scripts/login.sh --backup                 # Backup credential files
...
| `credentials.env` | project config/ | ~100B | Login/Password/Server for backtest.sh |
...
**Fix:** `login.sh` now saves password to `config/credentials.env` after successful auth. `backtest.sh` reads this file and includes `Password=` in the dynamically generated `[Common]` section. The Tester uses the password to authenticate without showing a popup.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Login popup during backtest | Missing Password in [Common] | Run `./scripts/login.sh` (saves credentials.env) |
| "account is not specified" | accounts.dat corrupt/missing | `./scripts/login.sh --restore` then re-login |
| "not synchronized" | servers.dat incomplete or common.ini missing | `./scripts/login.sh --restore` |
